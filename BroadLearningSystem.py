...
#     predlabel = predictLabel.flatten()
...
def BLS(train_x, train_y, test_x, test_y, s, c, N1, N2, N3):
    ...
    for i in range(N2):
        ...
        betaOfEachWindow = sparse_bls(FeatureOfEachWindowAfterPreprocess,
                                      FeatureOfInputDataWithBias).T
    ...
    InputOfEnhanceLayerWithBiasTest = np.hstack([OutputOfFeatureMappingLayerTest, 0.1 * np.ones(
        (OutputOfFeatureMappingLayerTest.shape[0], 1))])
    ...
    print('accurate2 is', testAcc * 100, '%')
    # print('Testing recall is', testrecall * 100, '%')
    # print('Testing f1_score is', f1_score * 100, '%')
    print('time is ', testTime, 's')
    test_acc[0][0] = testAcc
    test_time[0][0] = testTime
    # print(OutputOfTest)
    # def percentage_above_threshold(matrix, threshold):
    ...
    #
    # threshold = 100
    # abs_OutputWeight = np.abs(OutputWeight)
    # result = percentage_above_threshold(abs_OutputWeight, threshold)
    # print(f"大于{threshold}的元素占比：{result}%")
    # fig, ax = plt.subplots(figsize=(9, 6),dpi=600)
    # # 二维的数组的热力图，横轴和数轴的ticklabels要加上去的话，既可以通过将array转换成有column
    # # 和index的DataFrame直接绘图生成，也可以后续再加上去。后面加上去的话，更灵活，包括可设置labels大小方向等。
    # data=pd.DataFrame(np.round(OutputWeight, 2))
    # data.to_csv('data.csv', index=False)
    # sns.boxplot(pd.DataFrame(np.round(OutputWeight, 2)),)
    #
    # plt.ylim(-2000, 2000)
    # # ax.set_yticklabels([], fontsize=22, rotation=360, horizontalalignment='right')
    # plt.xticks([],fontsize = 18)
    #
    # # plt.savefig('left    brain', dpi=600)
    # plt.show()
    return testAcc, test_time, trainAcc, train_time
...
def BLS_AddFeatureEnhanceNodes(train_x, train_y, test_x, test_y, s, c, N1, N2, N3, L, M1, M2, M3):
    ...
    for e in list(range(L)):
        ...
        betaOfNewWindow = sparse_bls(FeatureOfEachWindowAfterPreprocess,
                                     FeatureOfInputDataWithBias).T
        ...
        outputOfNewWindow = (TempOfFeatureOutput - minOfEachWindow[N2 + e]) / distOfMaxAndMin[
            N2 + e]
        ...
        outputOfNewWindowTest = (outputOfNewWindowTest - minOfEachWindow[N2 + e]) / distOfMaxAndMin[
            N2 + e]
        ...
        InputOfOutputLayerTest = np.hstack(
            [InputOfOutputLayerTest, outputOfNewWindowTest, OutputOfRelateEnhanceNodes,
             OutputOfNewEnhanceNodes])
